...
def load_data(ticker):
    ...
    return X[:-n], X[-n:], yr[:-n], yr[-n:], yc[:-n], yc[-n:], scaler, df.index[-len(yr):], df, cidx
...
def run_job(job_id, ticker, mtype, bidir):
    try:
        ...
        yrp = np.sum([w[i]*reg_ms[i].predict(X_te, verbose=0) for i in range(len(reg_ms))], axis=0)
        ...
        frp = np.sum([w[i]*reg_ms[i].predict(X_te[-1:], verbose=0) for i in range(len(reg_ms))], axis=0)
        ...
        jobs[job_id]["result"] = {
            "ticker": ticker, "model_type": mtype, "bidirectional": bidir,
...
def start_predict(req: PredictReq, bg: BackgroundTasks):
    ...
    bg.add_task(run_job, jid, req.ticker.upper().strip(), req.model_type, req.bidirectional)
    return {"job_id": jid}
...
def get_status(job_id: str):
    ...
    j = jobs[job_id]
    return {"job_id": job_id, "status": j["status"], "result": j["result"], "error": j.get("error")}

def load_symbols_from_csv(path: str = "nifty.csv") -> list[dict]:
    """Read symbols from bundled nifty.csv, return [{label, value}]."""
    try:
        df = pd.read_csv(path)
        col = df.columns[0]
        syms = df[col].dropna().str.strip().tolist()
        # label = strip .NS suffix for readability
        return [{"label": s.replace(".NS", ""), "value": s} for s in syms if s]
    except Exception as e:
        return [{"label": "RELIANCE", "value": "RELIANCE.NS"}]

_SYMBOLS: list[dict] = load_symbols_from_csv()

@app.get("/symbols")
def symbols(q: str = ""):
    """Return all symbols, optionally filtered by query string."""
    data = _SYMBOLS
    if q:
        q_lower = q.lower()
        data = [s for s in data if q_lower in s["label"].lower() or q_lower in s["value"].lower()]
